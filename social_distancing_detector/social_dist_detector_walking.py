...
This script is best run when the view is from a person
walking along the street.

To use this function, run
python social_dist_detector_walking.py --input korea_trim.mp4 --output korea_trim_output_2.avi
'''
        ...
        if len(results) >= 2:
            ...
            for i in range(0, D.shape[0]):
                for j in range(i + 1, D.shape[1]):
                    ...
                    if (D[i, j] < results[i][3]):
                        # check to see if those people are in the same plane
                        # by comparing the bottom of their bounding box
                        if abs(results[i][1][1] - results[j][1][1]) < 18:
