...
def detect_and_predict_mask(frame, facenet, maskNet):
    # grab the dimensions of the frame nad then construct a blob
    # from it
    (h, w) = frame.shape[:2]
    ...
    # inialize the list of faces, corresponding locations, 
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > args['confidence']:
            ...
            face = cv2.img_to_array(face)
            face = preprocess_input(face)
            face = np.expand_dims(face, axis=0)

            # add the face and bounding boxes to respective
            # lists
            faces.append(face)
...
while True:
    # grab the grame from the threaded video stream and resize it
    # to have a maximum width of 400 pixels
    frame = vs.read()
    frame = imutils.resize(frame, width=400)

    # detect face in the frame and determine if they are wearing a 
    # face mask or not
